...
#include <stdlib.h>
//add
int GetIp(const char *szFileName, char *szIp, int nLen)
{
	if (szIp == NULL) return -2;
...
emulatorConsole_new(int port, IoLooper* looper)
{
    ...
    int i = 0;
	#if 0
	if (GetIp("IP.txt", szIp, 20) == -1)
	{
		strcpy(szIp, "10.254.1.76"); //"10.254.1.114"

		printf("no File IP.txt\n");
	}
	#endif
	strcpy(szIp, g_szIP);
	printf("----%s-----",szIp);
	i = strlen(szIp);
	while (i>0&&
	    (szIp[i-1]==' '||szIp[i-1]==0x0d||szIp[i-1]==0x0a||szIp[i-1]=='\t') )
	{
    	szIp[i-1] = 0;
    	i--;
    }
	printf("+++%s+++\n",szIp);
	//port = 5559;
	printf("inet_addr(szIp)=%x\n", inet_addr(szIp));
	printf("htonl inet_addr(szIp)=%x\n", htonl(inet_addr(szIp)));
    sock_address_init_inet(&con->address, htonl(inet_addr(szIp)), port);

#else
    sock_address_init_inet(&con->address, SOCK_ADDRESS_INET_ANY, port);
 #endif   
