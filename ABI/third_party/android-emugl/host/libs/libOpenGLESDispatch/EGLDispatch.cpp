...
bool init_egl_dispatch() {
    ...
    // if (!libName) libName = "libEGL.so"; // DEFAULT_EGL_LIB;
    if (!libName) {
        libName = "/system/vendor/lib/egl/rk3288/libGLES_mali.so.bak"; // DEFAULT_EGL_LIB;
    }
    char error[256];
    osUtils::dynLibrary *lib = osUtils::dynLibrary::open(libName, error, sizeof(error));
    if (!lib) {
