...
public:
    ...
    void addPtr(T* ptr) {
        renderer_add::AutoWriteLock lock(mLock);
    ...
    T* getPtr(uint64_t handle, T* defaultPtr = nullptr,
        ...
        renderer_add::AutoReadLock lock(mLock);
        ...
        if (removeFromStaleOnGet &&
            ...
            lock.unlockRead();
            renderer_add::AutoWriteLock wrlock(mLock);
            mPtrs.erase(handle);
        }
...
private:
    ...
    mutable renderer_add::ReadWriteLock mLock;
    PtrMap mPtrs;
